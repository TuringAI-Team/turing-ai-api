...
export async function autogenerateDocs(client) {
  ...
  for (let section of sections) {
    ...
    for (let model of client[section]) {
      ...
      //  pricing
      if (model.data.pricing) {
        docsModelContent += `\n## Pricing\n\n`;
        // average price
        if (model.data.pricing.average) {
          docsModelContent += `\n- **Average:** ${model.data.pricing.average}\n`;
        }
        // price
        if (model.data.pricing.price) {
      ...
      // response
      docsModelContent += `\n## Response\n\n`;
      if (model.data.response) {
        for (let response of Object.keys(model.data.response)) {
          docsModelContent += `\n- **${response}**\n  - Type: ${model.data.response[response].type}\n`;
          if (model.data.response[response].description) {
            docsModelContent += `  - Description: ${model.data.response[response].description}\n`;
          }
        }
      } else {
        //  alert not defined yet
        docsModelContent += `<Error>Response not defined yet</Error>\n\n`;
      }
