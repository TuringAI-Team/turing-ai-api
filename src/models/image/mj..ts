import client from "../../utils/selfbot.js";
...
export default {
  ...
  execute: async (data) => {
    let { prompt, model, action, number, id, stream } = data;
    if (!data.action) data.action = "imagine";
    if (!data.model) data.model = "5.2";
    if (!data.stream) data.stream = true;
    if (action == "imagine" || action == "variation") {
      let event = await queue(prompt, model, false, action, number, id);
      if (stream) {
        return event;
      } else {
...
const guildId = "1111700862868406383";
const botId = "936929561302675456";
...
export async function imagine(prompt, model, event, job) {
  ...
  const mjBot = botClient.users.cache.get(botId);
  ...
  event.on("close", async () => {
    ...
    await message.contextMenu(botId, "Cancel Job");
  });
